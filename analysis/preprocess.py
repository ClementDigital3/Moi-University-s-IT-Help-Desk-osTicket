...
from shared.text import clean_text, build_text

# Screening patterns. These decide which records are EXCLUDED from the corpus;
# they are a data-quality concern, not a text-representation one, so they stay
# here with the rest of the screening rules rather than in shared/text.py.
...
def main():
    ...
    df["text"] = [build_text(a, b) for a, b in zip(df["Subject"], df["Description"])]
