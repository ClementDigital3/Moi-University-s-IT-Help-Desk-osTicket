...
difference between the two arms is real or sampling noise.
"""
import os, json
import numpy as np
import pandas as pd
...
def load_splits():
    tr = pd.read_csv(os.path.join(PROC, "split_train.csv"), dtype=str).fillna("")
    te = pd.read_csv(os.path.join(PROC, "split_test.csv"), dtype=str).fillna("")
