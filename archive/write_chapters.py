...
def s45(w, R):
    ...
    for task in ("Ticket classification", "Resolver routing"):
        ...
        w.para(
            ...
            f"(McNemar exact p = {s['p-value']}). The advantage of {who} is therefore "
            f"distinguishable from sampling noise." if sig else
            f"(McNemar exact p = {s['p-value']}). The two approaches therefore cannot be "
            f"separated on this evidence.")
        w.para(
            f"Both arms are far above the majority-class baseline of "
            f"{f3(h['Majority baseline'])}, so both are learning the task rather than "
            f"exploiting the class prior.")
